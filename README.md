...
4. `npm run db:migrate:dev` - Creates a local SQLite database and applies migrations
5. `npm run dev`
...
2. Update the footer in `src/components/footer.tsx` with your project details and links
3. Optional: Update the color palette in `src/app/globals.css`
4. Update the metadata in `src/app/layout.tsx` with your project details
...
3. Create a Turnstile catcha in your Cloudflare account, and set the `NEXT_PUBLIC_CLOUDFLARE_TURNSTILE_SITE_KEY` as a Github Actions variable.
4. Set `CLOUDFLARE_TURNSTILE_SECRET_KEY` as a secret in your Cloudflare Worker.
